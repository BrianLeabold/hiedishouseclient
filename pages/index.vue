<template>
  <div class="container mb-3">
    <div>
      <h1 class="title">Heidi’s House - A Memorial Project</h1>
      <div class="content">
        <p class="mx-3">
          In memoriam of retired Navy Chief Hospital Corpsman (HMC), Heidi Sharon
          Amato, who passed after her 2nd battle with cancer in June 2019.
          “Heidi’s House” is a pilot program by W.A.G. Healing to explore and
          strengthen the Human and Animal bond toward healing. The first “tiny cabin” will
          be built to temporally house Mack & Stella (Heidi’s surviving furry animals) and
          provide space for both animals and visiting Volunteer Caretakers to heal and
          connect.
        </p>
      </div>
      <img
        class="mx-auto my-3"
        src="../static/Heidi'sHouse.png"
        title="Heidi's House Banner"
        alt="Heidi's House Banner"
      />
    </div>
  </div>
</template>

<script>
export default {
  head() {
    return {
      title: `About Heidi's House`,
      meta: [
        {
          hid: 'description',
          name: 'description',
          content: `“Heidi’s House” is a pilot program by W.A.G. Healing to explore and
          strengthen the Human and Animal bond toward healing.`
        },
        {
          hid: 'author',
          name: 'author',
          content: 'Brian Leabold'
        }
      ]
    }
  }
}
</script>

